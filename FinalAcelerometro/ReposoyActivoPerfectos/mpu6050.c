...
MSGQUEUE_MPU6050_OBJ_t txMsgMPU;
...
void Thread_MPU6050 (void *argument) {
  ...
  while (1) {
    ...
    if(comienzoLectura){
      txMsgMPU.ejeX = RD_ACCEL_MPU(ACCEL_XOUT_H);
      txMsgMPU.ejeY = RD_ACCEL_MPU(ACCEL_YOUT_H);
      txMsgMPU.ejeZ = RD_ACCEL_MPU(ACCEL_ZOUT_H);
      txMsgMPU.temp = RD_TEMP_MPU(TEMP_OUT_H);
    
      osMessageQueuePut(mid_MsgQueue_MPU, &txMsgMPU, 0U, 0U);
...
osMessageQueueId_t idQueueMPU (void){
  return mid_MsgQueue_MPU;
}
